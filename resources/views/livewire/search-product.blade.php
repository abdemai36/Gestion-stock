<div class="col-sm-9 col-md-6 col-lg-6 card ">
    <div class="p-4">
        ...
        <div class="d-flex">
            <a href="#" data-toggle="modal" id="add_cetegorieOrder" data-target="#exampleModalCenter" class="btn btn-success"><i class=" fas fa-plus-circle"></i></a>
            ...
            <div class="product-grid" >
                @foreach ($products as $product)
                    <div class="card addToSession" style="cursor: pointer">
                        <img class="card-img-top" src="{{ asset('product_image/'.$product->image) }}" alt="Card image cap">
                        <div class="card-body">
                            <input type="hidden" name="idProduct[]" class="idProduct" value="{{ $product->id }}">
                            <input type="hidden" name="percentageProduct[]" class="percentageProduct" value="{{ $product->percentage }}">
                            ...
                            <input type="hidden" name="priceProduct[]" class="priceProduct" value="{{ $product->price }}">
...
<script>
    ...
    $( "#ContentP" ).on('click','.addToSession',function() {
        ...
        $.ajax({
            ...
            data: {idProduct:idProduct,percentageProduct:percentageProduct,refProduct:refProduct,nameProduct:nameProduct,priceProduct:priceProduct,_token: '{{csrf_token()}}'},
       ...
       function selectDataProductSession()
                ...
                success: function (response) {
                    $.each(response.productSession, function (key,item) {
                        $('#ProductSession').append("<tr>"+
                            "<th scope='row'><input type='hidden' name='ref_product_added[]' value='"+item.ref+"' class='checkbox_ID' min=1 style='width: 60px'> <button data-id='"+item.product_ref+"' class='btn btn-light delete_ProdectSession'><i class='text-danger fas fa-trash'></i></button></th>"+
                            "<td><input type='hidden' name='name_product_added[]' value='"+item.name+"' class='checkbox_Name' min=1 style='width: 60px'>"+item.name+"</td>"+
                            "<td><input type='number' name='price_product_added[]' value='"+item.peice+"' class='checkbox_Price' min=1 style='width: 60px'></td>"+
                            "<td><input type='number' name='QNT_product_added[]' value='"+item.QNT+"' class='checkbox_QNT' min=1 style='width: 60px'></td>"+
                            "<td><input type='hidden' name='percentage_product_added[]' value='"+item.percentage+"' class='checkbox_percentage' min=1 style='width: 60px'>"+item.percentage+"</td>"+
                            "<td><input type='hidden' name='profit_product_added[]' value='"+item.profit+"' class='checkbox_profit' min=1 style='width: 60px'>"+item.profit+"</td>"+
                            "<td>"+item.total+"</td>"+
                            "</tr>"
                        );
                    });
            ...
            $("#ProductSession" ).on('focusout','.checkbox_QNT',function() {
                ...
                data: {valueQNT:valueQNT,product_ref:product_ref,valueProfit:valueProfit} ,
                success: function (data) {
                        if(data.code == 1)
                        {
                            toastr.success(data.msg);
            ...
            $("#ProductSession" ).on('focusout','.checkbox_Price',function() {
                ...
                data: {valueQNT:p_valueQNT,product_ref:p_product_ref,valueProfit:p_valueProfit,checkbox_Price:p_checkbox_Price} ,
                success: function (data) {
                        if(data.code == 1)
                        {
                            toastr.success(data.msg);
             ...
             function selectDataDepot()
            {
                ...
                success: function (response) {
                    ...
                    $.each(response.depots, function (key,item) {
                         $('#depotOrder').append("<option value='"+item.email+"'>"+item.name+"</option>");
                    });
                }
                });
            }
